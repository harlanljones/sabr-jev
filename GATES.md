...
- [x] G1: All offline Elixir application and latch tests pass without API key.
  ...
  EVIDENCE: 101 tests, 0 failures (env -u TYPESAFE_API_KEY mix test). Python ETL suite 23/23 OK.
...
- [x] G4: Real source lineage, recorded Jev responses, and recomputed cards verified.
  ...
  EVIDENCE: 35 real recordings (30 with retrospective oracle Noul + 4 qualified 2025 Choice/Score-only + Strider 2023 without oracle) validate with correct state_hash against cards.json; catalog provenance has manifest_sha256 + per-source sha256 + license. Latch v2 revision: act 0.65/review 0.45, Noul decoupled (original bars made act unreachable; documented in docs/evaluation.md with distribution analysis).
